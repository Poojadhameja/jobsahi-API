...
/* =========================================================
   POST METHOD: Schedule a new interview
   ========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ✅ Validate input
    if ($application_id <= 0 || $student_id <= 0) {
        echo json_encode(["status" => "error", "message" => "Missing or invalid application_id or student_id"]);
        exit();
    }
    ...
    try {
        // ✅ Step 1: Verify application belongs to this student
        $check_sql = "
            SELECT a.id, a.student_id, j.recruiter_id, j.title, u.user_name AS candidate_name, rp.company_name 
            FROM applications a
        ...
        if ($res->num_rows === 0) {
            echo json_encode(["status" => "error", "message" => "Invalid application_id or student_id"]);
            exit();
        }
        ...
        // ✅ Step 2: Verify recruiter ownership (if recruiter role)
        ...
        // ✅ Step 3: Insert new interview record
        $insert = $conn->prepare("
        ...
        // ✅ Step 4: Update application table (link interview)
        $update = $conn->prepare("
            UPDATE applications 
